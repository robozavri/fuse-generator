...
templateContent += `${key}: {
        ...
        case 'String':  templateContent += `${key}: '${key}'`;
          break;

        case 'Number':  templateContent += `${key}: _.random(1, 20)`;
          break;

        case 'imageSchema':  templateContent += `${key}: { url:  generateImage()}`;
          break;

        case 'Date': templateContent += `${key}: new Date()`;
          break;
...
get${_.upperFirst(key)}Object(i: number = 0): any {
    return {
        ${templateContent}
    };
};`;
